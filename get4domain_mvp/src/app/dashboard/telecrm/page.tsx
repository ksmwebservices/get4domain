...
export default function TeleCrmPage() {
  ...
  const selectLead = async (lead: CrmLead) => {
    setSelected(lead);
    setNotesDraft(lead.notes ?? '');
    try {
      const res = await api.getCrmLead(lead.id);
  ...
  const changeStatus = async (status: string) => {
    ...
    try {
      await api.updateCrmLead(selected.id, { status });
  ...
  const saveNotes = async () => {
    if (!selected) return;
    await api.updateCrmLead(selected.id, { notes: notesDraft });
    setSelected({ ...selected, notes: notesDraft });
  };

  const startCall = (lead: CrmLead) => {
    setSelected(lead);
    setInCall(true);
    setDuration(0);
    setOutcome(null);
  ...
  const runAiSummary = async () => {
    ...
    setSummarizing(true);
    try {
      const res = await api.generateAiCallSummary({ textNotes: callNotes, leadName: selected.name, callDuration: duration });
      setAiSummary(res.data?.summary ?? res.data?.content ?? String(res.data));
    } catch (e) {
  ...
  const saveCallLog = async () => {
    ...
    setSaving(true);
    try {
      await api.logCrmCall(selected.id, {
        duration,
  ...
  const onDrop = async (status: string, e: React.DragEvent) => {
    ...
    try { await api.updateCrmLead(id, { status }); } catch { load(); }
  };
  ...
  const QueueSection = ({ title, color, items }: { title: string; color: string; items: CrmLead[] }) => (
    <div>
  ...
  return (
    <div>
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-slate-900">TeleCRM</h1>
          <p className="text-sm text-slate-500">Call leads, log outcomes, move the pipeline.</p>
        </div>
