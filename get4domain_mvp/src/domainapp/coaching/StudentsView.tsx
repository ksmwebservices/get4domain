...
export default function StudentsView() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    api.getCoachingEnrollments().then((r) => setRows(r.data ?? [])).catch(() => setRows([])).finally(() => setLoading(false));
    api.getCoachingBatches().then((r) => setBatches((r.data ?? []).map((b: BatchRef) => ({ id: b.id, name: b.name })))).catch(() => setBatches([]));
  }, []);
  useEffect(() => { load(); }, [load]);
  ...
  async function save() {
    ...
    try {
      ...
      editing ? await api.updateCoachingEnrollment(editing.id, payload) : await api.createCoachingEnrollment(payload);
      setOpen(false); load();
    } finally { setSaving(false); }
  }
  async function remove(e: Enrollment) { if (confirm(`Delete enrollment for ${e.studentName}?`)) { await api.deleteCoachingEnrollment(e.id); load(); } }
      ...
      <Modal isOpen={open} onClose={() => setOpen(false)} title={editing ? 'Edit Enrollment' : 'Enroll Student'} maxWidth="max-w-lg">
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <Select label="Status" value={(form.status as string) ?? 'enrolled'} onChange={(e) => setForm({ ...form, status: e.target.value })}>{Object.entries(STATUS).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}</Select>
          </div>
