...
interface EngRef { id: string; title: string; clientName: string; engagementType: string }
interface DocItem { id: string; name: string; required: boolean; status: string; notes?: string; engagement?: EngRef | null }
...
export default function DocumentsView() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    api.getEngagementDocuments().then((r) => setRows(r.data ?? [])).catch(() => setRows([])).finally(() => setLoading(false));
  }, []);
  useEffect(() => { load(); }, [load]);
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-0.5 text-sm text-slate-500">Every required document across engagements — chase what&apos;s outstanding.</p>
      </div>
      ...
      {loading ? <div className="py-16 text-center text-sm text-slate-400">Loading…</div>
        : filtered.length === 0 ? <Card padded><EmptyState icon="FileCheck2" title={rows.length ? 'Nothing here' : 'No documents yet'} subtitle={rows.length ? 'No documents in this state.' : 'Documents appear as you open engagements.'} /></Card>
        : (
                  ...
                  <button onClick={() => cycle(d)} className="flex min-w-0 items-center gap-3 text-left">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="truncate text-xs text-slate-500">
                        {d.engagement ? <>{d.engagement.title} · {d.engagement.clientName}</> : 'Unlinked'}
                        {d.engagement && <span className="ml-1.5 rounded bg-slate-100 px-1.5 py-0.5 text-[10px] font-medium text-slate-500">{d.engagement.engagementType}</span>}
                      </div>
                    </div>
                  </button>
